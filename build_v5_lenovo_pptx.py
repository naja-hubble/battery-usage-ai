#!/usr/bin/env python
"""FCC更新停止者の判定技術 — 社内 patent review 用サマリ deck v5 Lenovo版 (.pptx).

Lenovo_Restricted-Template.potx 由来のマスター/レイアウト(ロゴ・Restricted透かし・
フッター)の上に v5 と同一コンテンツを構築する。
...
A2, A3, B, C2, C3, D, DM, E_ = (R["A2"], R["A3"], R["B"], R["C2"], R["C3"],
                                R["D"], R["Dmin"], R["E"])
FIG = pc.FIG_DIR
OUT = pc.REPORTS / "fcc_patent_summary_slides_v5_lenovo.pptx"
TEMPLATE = pc.REPORTS / "temp" / "_restricted_base.pptx"  # COM-converted from Lenovo_Restricted-Template.potx
...
LENOVO_RED = RGBColor(0xE1, 0x25, 0x1B)   # sampled from template logo
PLUM = RGBColor(0x1E, 0x01, 0x13)         # template dark background
FONT = "Meiryo UI"      # East-Asian font
LATIN = "Arial"         # template latin font
...
for _sld in list(_sldIdLst):          # drop the template's sample slides
    prs.part.drop_rel(_sld.get(qn("r:id")))
    _sldIdLst.remove(_sld)
...
BLANK = _layout("Blank Slide")
TITLE_ONLY = _layout("Title Only")
TITLE_BLACK = _layout("Title Slide_Black")
...
def section(title, accent=BLUE, tag=None):
    ...
    ph = s.shapes.title
    ph.left, ph.top = EM(Inches(0.45)), EM(Inches(0.16))
    ph.width, ph.height = EM(SW - Inches(3.7)), EM(Inches(0.72))
    ph.text = title
    ph.text_frame.word_wrap = True
    for run in ph.text_frame.paragraphs[0].runs:
        _style(run, 21, True, PLUM)
    line_seg(s, Inches(0.45), Inches(0.98), SW - Inches(0.45), Inches(0.98),
             color=LENOVO_RED, width=2.0)
    if tag:
        chip = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE,
                                  EM(SW - Inches(3.15)), EM(Inches(0.26)),
                                  EM(Inches(2.75)), EM(Inches(0.44)))
        chip.fill.solid(); chip.fill.fore_color.rgb = accent
        chip.line.fill.background(); chip.shadow.inherit = False
        chip.text_frame.margin_top = Pt(1); chip.text_frame.margin_bottom = Pt(1)
        _set(chip.text_frame.paragraphs[0], tag, size=10.5, bold=True,
             color=WHITE, align=2)
    return s
...
def takeaway(slide, text, y=None):
    ...
    shp.fill.solid(); shp.fill.fore_color.rgb = RGBColor(0xF4, 0xF4, 0xF4)
    shp.line.color.rgb = LENOVO_RED; shp.shadow.inherit = False
    tf = shp.text_frame; tf.word_wrap = True
    tf.margin_top = Pt(2); tf.margin_bottom = Pt(2)
    _set(tf.paragraphs[0], "💡 " + text, size=11.5, bold=True,
...
# 1. Title
# =========================================================================== #
s = prs.slides.add_slide(TITLE_BLACK)
_t = s.shapes.title
_t.left, _t.top = EM(Inches(0.8)), EM(Inches(1.62))
_t.width, _t.height = EM(SW - Inches(2.6)), EM(Inches(1.85))
_t.text = "テレメトリからの FCC更新停止（ゲージ凍結）者の判定技術"
_t.text_frame.word_wrap = True
_t.text_frame.vertical_anchor = MSO_ANCHOR.TOP
for run in _t.text_frame.paragraphs[0].runs:
    _style(run, 30, True, WHITE)
for _ph in s.placeholders:
    if _ph.placeholder_format.idx != 0:
        _ph.left, _ph.top = EM(Inches(0.8)), EM(Inches(3.42))
        _ph.width, _ph.height = EM(SW - Inches(2.6)), EM(Inches(0.55))
        _ph.text = "— と、その実現過程で生まれた特許候補アイデア —  |  社内 patent review 用 v5 (Lenovo Restricted)  |  2026-07-06"
        _ph.text_frame.vertical_anchor = MSO_ANCHOR.TOP
        for run in _ph.text_frame.paragraphs[0].runs:
            _style(run, 15, False, RGBColor(0xE8, 0xE0, 0xE4))
tf2 = _tb(s, Inches(0.8), Inches(4.15), SW - Inches(1.6), Inches(2.2))
for i, ln in enumerate([
    ...
    _set(p, "•  " + ln, size=13.5, color=WHITE); p.space_after = Pt(4)
_dtf = _tb(s, Inches(0.8), Inches(6.28), SW - Inches(1.6), Inches(0.32))
_set(_dtf.paragraphs[0], DISC, size=8, color=RGBColor(0x9A, 0x8F, 0x96))
...
_set(tf.paragraphs[0],
     "読み方: 各スライド下部のグレー帯（💡）は非エンジニア向けの一言要約。図スライドは4ブロック"
